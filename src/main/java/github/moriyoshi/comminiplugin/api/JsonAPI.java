...
public abstract class JsonAPI extends FileAPI<JsonElement> {
  ...
  public static JsonElement convertElement(Object obj) {
    return obj instanceof JsonElement ? ((JsonElement) obj) : ComMiniPlugin.gson.toJsonTree(obj);
  }
   ...
   */
  public static @Nullable JsonObject getExistJsonObject(
      @NotNull JsonObject json, List<Object> path) {
   ...
   * @see #getJsonObject(JsonObject, List, boolean)
   */
  public static @NotNull JsonObject getCreateJsonObject(
      @NotNull JsonObject json, List<Object> path) {
    return Objects.requireNonNull(getJsonObject(json, path, true));
  }

  /**
   * jsonからpathの通りにして{@link JsonObject} を取得します creatableがtrueの場合に
   *
   * <p>pathがない場合は空のJsonObjectを追加します
   *
   * @param json 取得する対象
   * @param path ["data",1,] など
  ...
  public static JsonObject getJsonObject(
      @NotNull JsonObject json, List<Object> path, boolean creatable) {
